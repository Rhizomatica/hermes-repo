...
repo_has_binaries_for_sourcever_arch() {
  ...
  reprepro -b "$REPO_DIR" --ignore=unknownfield --ignore=undefinedtarget -T deb -A "$arch" --list-max 1 listfilter "$CODENAME" "$formula" 2>/dev/null | grep -q .
}

repo_has_sourcever() {
  local src="$1"
  local ver="$2"
  reprepro -b "$REPO_DIR" --ignore=unknownfield --ignore=undefinedtarget -T dsc --list-max 1 listfilter "$CODENAME" "Package (== $src), Version (== $ver)" 2>/dev/null | grep -q .
}
...
replace_existing_binaries_for() {
  ...
  for a in $arch_list; do
    ...
    reprepro -b "$REPO_DIR" --ignore=unknownfield --ignore=undefinedtarget --export=silent-never -T deb -A "$a" removefilter "$CODENAME" "$formula" >/dev/null 2>&1 || true
    reprepro -b "$REPO_DIR" --ignore=unknownfield --ignore=undefinedtarget --export=silent-never -T ddeb -A "$a" removefilter "$CODENAME" "$formula" >/dev/null 2>&1 || true
  done
...
include_changes() {
  ...
  for ch in "${changes[@]}"; do
    ...
    if out="$(reprepro -b "$REPO_DIR" --ignore=unknownfield --ignore=undefinedtarget --export=silent-never --ignore=wrongdistribution include "$CODENAME" "$ch" 2>&1)"; then
      ec=0
    else
      ec=$?
    fi
...
reprepro -b "$REPO_DIR" --ignore=unknownfield --ignore=undefinedtarget export "$CODENAME"
